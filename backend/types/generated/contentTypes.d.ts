...
export interface ApiBlogPostBlogPost extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: "Uses Strapi's own draft and publish rather than a status field. Every post is two versions of one document, and the API serves the published one by default - which is exactly the rule the spec asks for, so there is no second source of truth to keep in step.";
...
export interface ApiCourseCourse extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A course groups the lessons a student works through.';
    displayName: 'Course';
    pluralName: 'courses';
    singularName: 'course';
  };
...
export interface ApiEnrollmentEnrollment extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'Links a student to a course they signed up for. One row per student per course - the controller rejects duplicates, since a composite unique constraint cannot be expressed in the schema.';
...
export interface ApiLessonProgressLessonProgress
  ...
  info: {
    description: 'One row per lesson a student has marked complete. The row existing is what complete means - there is no boolean flag that could drift out of sync with reality. Un-marking a lesson deletes the row.';
...
export interface ApiLessonLesson extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'One unit of a course: written content and/or a video.';
    displayName: 'Lesson';
    pluralName: 'lessons';
    singularName: 'lesson';
  };
...
export interface ApiQuizAttemptQuizAttempt extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A graded submission. Unlike lesson progress, the score and percentage are stored rather than recomputed: an attempt is a record of how a student did against the quiz as it stood that day, and editing the quiz later must not rewrite history.';
...
export interface ApiQuizQuiz extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'An MCQ quiz attached to a course. Questions are components rather than their own collection, so a quiz is created, edited and versioned as one document.';
    displayName: 'Quiz';
    pluralName: 'quizzes';
    singularName: 'quiz';
  };
